...
export default function Login({ onLogin, onBack }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!success) {
      setError('Invalid credentials. Try admin / magic2024');
      ...
      <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className="w-full magic-gradient text-white py-3.5 rounded-xl shadow-magic hover:shadow-magic-lg transition-all text-base sm:text-lg"
          >
            Login to Dashboard
          </motion.button>
        </form>
